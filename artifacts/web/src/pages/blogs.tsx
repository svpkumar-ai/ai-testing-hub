import { useGetBlogs, getGetBlogsQueryKey } from "@workspace/api-client-react";
...
export default function BlogsPage() {
  ...
  const { data, isLoading, isError, refetch, isRefetching } = useGetBlogs(
    { page: 1, limit: 100 },
    {
      query: {
        queryKey: getGetBlogsQueryKey({ page: 1, limit: 100 }),
        staleTime: 1000 * 60 * 1,
      },
    }
  );
  ...
  async function handleForceRefresh() {
    await fetch("/api/blogs?force=true");
  ...
  const filteredArticles = useMemo(() => {
    if (!data?.articles) return [];
    if (!selectedSource) return data.articles;
    return data.articles.filter((a) => a.source === selectedSource);
  }, [data?.articles, selectedSource]);
            ...
            >
              ...
              {data?.articles && (
                <span className={cn(
                  ...
                  !selectedSource ? "bg-primary/20 text-primary" : "bg-white/10 text-muted-foreground"
                )}>
          ...
          <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-10 gap-4">
              ...
              >
                <h1 className="text-4xl font-display font-bold text-foreground tracking-tight">
                  {selectedSource ?? "QA & Testing Blogs"}
                </h1>
                <p className="text-lg text-muted-foreground max-w-2xl">
                  {selectedSource
                    ? `${filteredArticles.length} article${filteredArticles.length !== 1 ? "s" : ""} from this blog`
                    : "Latest articles from top QA and software testing blogs — covering AI, automation, tools, and best practices."}
                </p>
              </motion.div>
            ...
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-32 opacity-50">
                ...
                <p className="text-lg font-medium">Fetching the latest articles...</p>
              </div>
            ) : isError ? (
              <div className="flex flex-col items-center justify-center py-20 bg-destructive/5 rounded-3xl border border-destructive/10">
                ...
                <p className="text-muted-foreground mb-6 text-center max-w-md">
                  We couldn&apos;t reach the blog sources. Please try again later.
                </p>
